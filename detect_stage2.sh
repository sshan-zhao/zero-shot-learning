...
save_file="detect_results/$dataset/${cls}_test/ATTR_PRED_${resize}x${resize}_iter_$iter"
model_file="models/$dataset/${cls}_$resize/ATTR_PRED_${resize}x${resize}"
...
python zsl_detect.py --gpu_id 0 --model_def $model_file/deploy.prototxt \
		...
		--attribute_file $train_file/attributes.txt --result_file ${cls}.txt\
